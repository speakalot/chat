...
var readOpts		= { highWaterMark: Math.pow(2, 128), objectMode: true};
var writeOpts		= { highWaterMark: Math.pow(2, 128), objectMode: true};
	...
	io.sockets.on('connection', function (socket) {
		...
		socket.on('sendMessage', function (data, fn) {
			...
			if (usr == undefined) {
				...
				for (x in usr.socketId) {
					io.to(usr.socketId[x]).emit('receiveMessage', { msg: data.msg, 
																	remetente: socket.user.userid, 
																	nick: socket.user.username, 
																	type: data.type, 
		...
		ss(socket).on('sendFile', function (stream, data) {
			...
			try {
				...
				var newStream = ss.createStream(writeOpts)
